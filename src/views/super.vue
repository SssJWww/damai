<template>
  <div>
    <pubhead @event="handleEvent">超级券</pubhead>
...
export default {
  ...
  methods: {
    ...
    getData (id) {
      Axios.post(
        '/api/goods/goodsList',
        `cid=0&sort=${id}&type=supercoupon&page_num=1&page_size=100`
      ).then(res => {
    ...
    handleMore () {
      ...
      Axios.post(
        '/api/goods/goodsList',
        `cid=0&sort=${this.id}&type=supercoupon&page_num=${this.current}&page_size=100`
      ).then(res => {
        // console.log(res.data.data.list)
